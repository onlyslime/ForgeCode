...
export const sessionWait = (session, { workspace, timeout, ...options } = {}) => { validateSession(session); validateWorkspace(workspace); if (timeout !== undefined && (typeof timeout !== "number" || !Number.isFinite(timeout) || timeout < 0 || timeout > 60)) throw new TypeError("timeout must be between 0 and 60 seconds"); return method("session.wait", { ...options, params: { ...(options.params ?? {}), session, ...(workspace === undefined ? {} : { workspace }), ...(timeout === undefined ? {} : { timeout }) } }); };
export const sessionEvents = (session, { workspace, after, limit, wait, type, ...options } = {}) => { validateSession(session); validateWorkspace(workspace); if (after !== undefined && (typeof after !== "number" || !Number.isInteger(after) || after < 0)) throw new TypeError("after must be a non-negative integer"); if (limit !== undefined && (typeof limit !== "number" || !Number.isInteger(limit) || limit < 1 || limit > 100)) throw new TypeError("limit must be between 1 and 100"); if (wait !== undefined && (typeof wait !== "number" || !Number.isFinite(wait) || wait < 0 || wait > 30)) throw new TypeError("wait must be between 0 and 30 seconds"); if (type !== undefined && (typeof type !== "string" || !type || type.length > 64 || /[\r\n]/.test(type))) throw new TypeError("type must be bounded newline-safe text"); return method("session.events", { ...options, params: { ...(options.params ?? {}), session, ...(workspace === undefined ? {} : { workspace }), ...(after === undefined ? {} : { after }), ...(limit === undefined ? {} : { limit }), ...(wait === undefined ? {} : { wait }), ...(type === undefined ? {} : { type }) } }); };
...
export const providerList = ({ workspace, ...options } = {}) => { validateWorkspace(workspace); return method("provider.list", { ...options, params: { ...(options.params ?? {}), ...(workspace === undefined ? {} : { workspace }) } }); };
export const providerHealth = ({ workspace, ...options } = {}) => { validateWorkspace(workspace); return method("provider.health", { ...options, params: { ...(options.params ?? {}), ...(workspace === undefined ? {} : { workspace }) } }); };
