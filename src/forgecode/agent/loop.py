...
class AgentLoop:
    def __init__(
        ...
        self.context = ToolContext(context.guard, context.approval, approval_observer=record_approval)
    ...
    async def run(self, prompt: str) -> LoopResult:
        ...
        messages: list[Message] = [
            self.context_builder.system_message(self.context.guard.root, self.registry.names(), approval_mode="interactive or explicit auto-approve"),
            Message(role="user", content=prompt),
        ]
        self._record("user_message", {"content": prompt})
        ...
        for step in range(self.config.max_steps):
            ...
            try:
                response = await self.provider.complete(request_messages, self.registry.schemas())
            except (KeyboardInterrupt, asyncio.CancelledError):
                error_text = "agent interrupted by user"
                self._record("final", {"stopped_reason": "interrupted", "error": error_text})
                return LoopResult(tuple(messages), "interrupted", error_text, verification_ok)
            except ProviderError as exc:
                error_text = str(exc)
                self._record("error", {"category": exc.category, "message": error_text})
                result = LoopResult(tuple(messages), "provider_error", error_text, verification_ok)
            ...
            if not is_valid_response(response):
                ...
                result = LoopResult(tuple(messages), "invalid_response", error_text, verification_ok)
                self._record("final", {"stopped_reason": result.stopped_reason, "error": error_text})
                return result
            ...
            if not response.message.content and not response.message.tool_calls:
                ...
                result = LoopResult(tuple(messages), "empty_response", error_text, verification_ok)
                self._record("final", {"stopped_reason": result.stopped_reason, "error": error_text})
                return result
            ...
            if not response.message.tool_calls:
                if self.config.verification_command and verification_ok is not True and verification_runs < self.config.max_verification_attempts:
                    ...
                    if not verification.ok and verification_runs >= self.config.max_verification_attempts:
                        ...
                        result = LoopResult(tuple(messages), "verification_failed", error_text, False)
                        self._record("final", {"stopped_reason": result.stopped_reason, "error": error_text, "verification_ok": False})
                        return result
                    continue
                result = LoopResult(tuple(messages), "model_finished", verification_ok=verification_ok)
            ...
            for call in response.message.tool_calls:
                ...
                if seen_calls[fingerprint] > self.config.max_repeated_calls:
                    ...
                    result = LoopResult(tuple(messages), "repeated_tool_call", output, verification_ok)
        ...
        result = LoopResult(tuple(messages), "max_steps", error_text, verification_ok)
