...
# Machine-readable safety negotiation.  These are capabilities, not grants:
        ...
        try:
            ...
            if method is not None:
                ...
                if method == "session.list":
                    ...
                    with _SESSION_LOCK:
                        ...
                        if info.get("mode") == "act":
                            try:
                                ...
                                trusted = False
                            if not trusted and method not in {"session.cancel", "session.result", "session.wait"}:
                    ...
                    if method == "session.events":
                        ...
                        if isinstance(wait_seconds, bool) or not isinstance(wait_seconds, (int, float)) or wait_seconds < 0 or wait_seconds > 30:
                            raise ValueError("session.events.wait must be between 0 and 30 seconds")
            ...
            if method in {"run", "session.run"} and handle and background:
                with _SESSION_LOCK:
                    ...
                    info["worker"] = worker
                    info["execution"] = "process" if isolate else "thread"
                    worker.start()
                    _persist_session(handle, info)
