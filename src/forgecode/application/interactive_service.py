...
class InteractiveSession:
    ...
    def introduction_text() -> str:
        return (
            "ForgeCode\n"
            "═════════\n"
            "ForgeCode 是一个本地运行、可审计、面向真实软件工程工作的 coding agent。它不是把现成 agent 产品换一层外壳，而是自己实现模型协议、对话循环、工具注册、本地执行、上下文管理、错误恢复和安全边界。你只需要用自然语言描述目标，它会在工作区中调查、思考、行动、验证，并把过程完整呈现出来。\n\n"
            "它能做什么\n"
            "──────────\n"
            ...
            "它如何工作\n"
            "──────────\n"
            "每一轮都遵循“模型决定下一步 → 本地工具执行 → 结果回到模型”的闭环。模型可以连续进行多轮调查和修改，也可以并行发起多个只读工具调用；每个 turn、provider attempt、重试、工具调用、工具结果和最终状态都会写入会话事件流。界面会实时显示 Understand、Inspect、Modify、Verify 阶段、模型增量输出、工具参数摘要、文件内容预览、diff、耗时和验证结果，而不是只给一个黑盒答案。\n\n"
            "上下文与会话\n"
            "────────────\n"
            "ForgeCode 提供增量上下文索引、语言和符号统计、陈旧条目诊断、上下文搜索、自动压缩和可解释的上下文摘要，避免大型项目把模型上下文迅速耗尽。会话以 JSONL 事件持久化，支持状态查询、事件回放、session tree、分支、检查点、恢复、审查和可验证的事务回滚；即使任务中断，也不会把未完成操作伪装成成功。\n\n"
            "安全与可靠性\n"
            "────────────\n"
            "所有文件路径都经过 WorkspaceGuard 校验，禁止越出工作区；写入和命令执行受 plan / act / bypass 模式、workspace trust、风险分类、审批策略、超时和取消控制。支持 Esc 取消、pause/resume、provider 重试、超时恢复、未决副作用检测和审计记录。凭据只用于请求认证并在日志、上下文、错误和输出中自动脱敏。\n\n"
            "模型与自动化\n"
            "────────────\n"
            "通过统一的 provider-neutral 协议连接 OpenAI-compatible、Anthropic、Google、Ollama 等模型后端，兼容 JSON 与 SSE 流式 tool calling，并校验工具 ID、参数、finish reason 和响应大小。除了交互式 fcc，还支持 --print、--json、--jsonl、RPC 和 Python embed API，可以把同一个 agent 接入脚本、CI 或其他界面。\n\n"
            "适合的任务\n"
            "──────────\n"
            "从修复一个 bug、补充测试、重构模块、分析日志，到跨文件迁移、运行验证、审查变更和恢复中断会话，ForgeCode 都会尽量把“为什么这样做、做了什么、结果如何”留在可检查的证据链中。\n\n"
            "详见 GitHub：\n"
