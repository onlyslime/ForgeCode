...
def test_workspace_summary_reports_expired_deadline(tmp_path: Path):
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    result = registry.execute("workspace_summary", {}, context)
    assert result.ok is False
    assert result.metadata["error"] == "deadline_exceeded"
...
def test_listing_and_search_report_expired_deadline(tmp_path: Path):
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    for name, arguments in (("list_files", {"pattern": "*"}), ("search", {"query": "x"})):
...
def test_map_and_metadata_report_expired_deadline(tmp_path: Path):
    (tmp_path / "sample.py").write_text("x = 1", encoding="utf-8")
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    for name, arguments in (("repository_map", {}), ("file_metadata", {"path": "sample.py"})):
        result = registry.execute(name, arguments, context)
        assert result.ok is False
        assert result.metadata["error"] == "deadline_exceeded"


def test_understanding_tools_report_expired_deadline(tmp_path: Path):
    (tmp_path / "sample.py").write_text("def sample():\n    return 1\n", encoding="utf-8")
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    for name, arguments in (("read_range", {"path": "sample.py", "start_line": 1, "end_line": 1}), ("list_symbols", {"path": "sample.py"})):
        result = registry.execute(name, arguments, context)
        assert result.ok is False
        assert result.metadata["error"] == "deadline_exceeded"
...
def test_git_worktree_listing_reports_expired_deadline(tmp_path: Path):
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    result = registry.execute("git_worktrees", {}, context)
    assert result.ok is False
    assert result.metadata["error"] == "deadline_exceeded"


def test_worktree_mutations_report_expired_deadline(tmp_path: Path):
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    result = registry.execute("git_worktree_create", {"name": "expired", "branch": "expired"}, context)
    assert result.ok is False
    assert result.metadata["error"] == "deadline_exceeded"


def test_git_commit_reports_expired_deadline(tmp_path: Path):
    registry = build_default_registry(WorkspaceGuard(tmp_path))
    context = ToolContext(WorkspaceGuard(tmp_path), AllowAllApproval(), deadline_monotonic=time.monotonic() - 1)
    result = registry.execute("git_commit", {"message": "expired"}, context)
    assert result.ok is False
    assert result.metadata["error"] == "deadline_exceeded"


def test_apply_patch_reports_expired_deadline(tmp_path: Path):
